...
We will prepare scRNAseq data corresponding to differentiating sperm cells, generated in `Hermann et al., Cell Rep. 2018` (doi: 10.1016/j.celrep.2018.10.026). 
...
### Visualization of clustered and embedded data

- Is the resulting object comparable to the published set (check their published paper to see how their embedding looks like)? What changed? Which information was provided by the authors and retained? Comment on the `DiffusionMap` embedding vs `PCA`. 

<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce
require(patchwork)
p<- scater::plotReducedDim(sce, 'PCA', colour_by = 'AnnotatedClusters') + ggtitle('PCA') + 
    scater::plotReducedDim(sce, 'DiffusionMap', colour_by = 'AnnotatedClusters') + ggtitle('DiffusionMap')
```
</p></details><br>
...
A nice breakdown (from 2019, so already very outdated!) is available from `Saelens et al., Nat. Biotech. 2018` (doi: 10.1038/s41587-019-0071-9): 
...
sce_slingshot <- slingshot::slingshot(
    sce, 
    reducedDim = 'PCA', 
    clusterLabels = sce$AnnotatedClusters
)
```

- Check the inferred trajectory(ies). Do they fit your expectations? Try changing the different parameters of `slingshot`. What do you observe? Discuss. 

<details><summary>Show code</summary><p>
```{r eval = FALSE}
inferred_traj <- slingshot::slingCurves(sce_slingshot)[[1]]
names(inferred_traj)
pca_curve <- as.data.frame(inferred_traj$s)
p <- tibble(
    x = reducedDim(sce, 'PCA')[,1], 
    y = reducedDim(sce, 'PCA')[,2], 
    cluster = sce$AnnotatedClusters
) %>% 
    ggplot() + 
...
sce_slingshot2 <- slingshot::slingshot(
    sce, 
    reducedDim = 'PCA', 
    clusterLabels = sce$AnnotatedClusters, 
    approx_points = 200, 
...
pca_curve2 <- as.data.frame(slingshot::slingCurves(sce_slingshot2)[[1]]$s)
p <- tibble(
    x = reducedDim(sce, 'PCA')[,1], 
    y = reducedDim(sce, 'PCA')[,2], 
    cluster = sce$AnnotatedClusters
    ...
    ggplot() + 
    geom_point(aes(x, y, col = cluster)) + 
    geom_path(data = pca_curve2, aes(x = PC1, y = PC2)) + 
    theme_minimal() + 
    coord_fixed()
```
</p></details><br>

- Project the trajectory in one of the available non-linear embeddings. Can you comment on the qualitative differences when observing the trajectory in linear or non-linear embeddings? 
...
inferred_traj <- slingshot::slingCurves(sce_slingshot)[[1]]
embedded_traj <- slingshot::embedCurves(sce_slingshot, "DiffusionMap")
diffusion_curve <- as.data.frame(slingshot::slingCurves(embedded_traj)[[1]]$s[slingshot::slingCurves(embedded_traj)[[1]]$ord, ])
p <- tibble(
    ...
    ggplot() + 
    geom_point(aes(x, y, col = cluster)) + 
    geom_path(data = diffusion_curve, aes(x = DC1, y = DC2)) + 
    theme_minimal() + 
    coord_fixed()
```
</p></details><br>
...
More TI algorithms can now be explored within `R`, even those relying on `Python` scripts, thanks to the `dynverse` package. 

The `dynverse` facilitates trajectory inference with 50+ different algorithms, using a unified syntax. Here are the main steps of the recommended workflow: 
...
dataset <- add_grouping(
    ...
    tibble(
        ...
        group_id = sce$AnnotatedClusters
    )
)
...
<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce$pseudotime <- slingshot::slingPseudotime(sce_slingshot2)[, 'Lineage1']
p <- tibble(
    cluster = sce$AnnotatedClusters, 
...
<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce$pseudotime <- (-1 * slingshot::slingPseudotime(sce_slingshot)[, 'Lineage1']) %>% 
    scales::rescale(c(0, 1))
```
</p></details><br>

#### Model pseudotime-dependent gene expression

Using pseudotime / cell, one can model gene expression along the differentiation process. This alleviates the need to 
study gene expression **per cell**, and allows one to focus on process-related effects (e.g. gene expression during a developmental trajectory). 

- Try to do so for few markers of spermatogonia, spermatocytes and spermatids. You can use different models to 
...
p <- ggplot(fitExprs) + 
    ...
    # geom_line(aes(x = pseudotime, y = lmFitted_expr), col = 'white', size = 2, alpha = 0.5) + 
    # geom_line(aes(x = pseudotime, y = lmFitted_expr), col = '#1b8ae6', size = 1) +
    # geom_line(aes(x = pseudotime, y = loessFitted_expr), col = 'white', size = 2, alpha = 0.5) + 
    # geom_line(aes(x = pseudotime, y = loessFitted_expr), col = '#0d7c2e', size = 1) +
...
p <- scater::plotReducedDim(sce, 'PCA', colour_by = "AnnotatedClusters", point_alpha = 0.5) +
    geom_segment(
        data = grid.df, 
...
- RNA velocity also yields a pseudotime. How does it compare to pseudotime computed by `slingshot`? 

<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce$pseudotime_velocity <- velo_out$velocity_pseudotime
p <- tibble(
    slingshot = sce$pseudotime, 
    velocity = sce$pseudotime_velocity, 
    cluster = sce$AnnotatedClusters
) %>% 
    ggplot(aes(x = slingshot, y = velocity, col = cluster)) + 
    geom_point() + 
    theme_minimal()
```
</p></details><br>

- Try running `velociraptor` after removing the cells in the center of the projection. Re-run RNA velocity: did it change the trajectory of individual cells? Comment. 

<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce2 <- sce[, sce$pseudotime_velocity <= 0.625]
velo_out2 <- velociraptor::scvelo(
    sce2, 
    assay.X = "spliced", 
    subset.row = HVGs[1:100], 
    use.dimred = "PCA", 
...
sce2$pseudotime_velocity <- velo_out2$velocity_pseudotime
embedded_velo2 <- velociraptor::embedVelocity(reducedDim(sce2, "PCA"), velo_out2)
grid.df2 <- velociraptor::gridVectors(reducedDim(sce2, "PCA"), embedded_velo2, resolution = 30)
p <- scater::plotReducedDim(sce2, 'PCA', colour_by = "pseudotime_velocity", point_alpha = 0.5) +
    geom_segment(
        data = grid.df2, 
        mapping = aes(x = start.PC1, y = start.PC2, xend = end.PC1, yend = end.PC2), 
        arrow = arrow(length = unit(0.05, "inches"), type = "closed")
...
## 3. Differential expression along differentiation trajectory

### Model gene expression 

Gene expression can be modelled as a function of pseudotime using GAM. 
This can be done manually using the `mgcv` package (see hereabove), but for scRNAseq, 
...
p <- cowplot::plot_grid(
    plotlist = lapply(1:16, function(K) {
        scater::plotReducedDim(
            sce, 
            'DiffusionMap',
            colour_by = rownames(res)[K], 
            point_alpha = 0.3
        ) + 
        theme_void() + 
        ggtitle(rownames(res)[K]) + 
        scale_color_gradientn(colours = c('#ffecae', '#ffc46c', '#ff7e14', '#c9430f', '#8d0909')) + 
        theme(legend.position = 'none')
    })
)
res <- sce_test %>% drop_na() %>% arrange(desc(meanLogFC)) %>% tail(16)
p <- cowplot::plot_grid(
    plotlist = lapply(1:16, function(K) {
        scater::plotReducedDim(
            sce, 
...
<details><summary>Show code</summary><p>
```{r eval = FALSE}
sce_test <- tradeSeq::startVsEndTest(
    sce_fit,
    l2fc = 1, 
    pseudotimeValues = c(0.5, 0.8)
)
res <- sce_test %>% drop_na() %>% arrange(desc(logFClineage1)) %>% head(16)
p <- cowplot::plot_grid(
    plotlist = lapply(1:16, function(K) {
        scater::plotReducedDim(
            sce, 
...
res <- sce_test %>% drop_na() %>% arrange(desc(logFClineage1)) %>% tail(16)
p <- cowplot::plot_grid(
    plotlist = lapply(1:16, function(K) {
        scater::plotReducedDim(
            sce, 
