...
rowData(sce)$ID <- scuttle::uniquifyFeatureNames(rowData(sce)$Symbol_TENx, rowData(sce)$ENSEMBL_ID)
...
ggplot(df) + 
    geom_point(aes(x = mean, y = var, col = HVG)) + 
    geom_line(aes(x = mean, y = trend)) +
    theme_minimal()
```
...
Fill in the analysis template in `bin/prepare_Ernst.R` to execute the different 
processing/analysis steps we covered in this exercise. 
