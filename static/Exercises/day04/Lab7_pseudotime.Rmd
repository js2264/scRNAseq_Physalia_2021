...
#### Daunting step but that looks cool in a paper: modeling pseudotime-dependent gene expression
...
- For those who want to go further... Try running `velociraptor` after removing the cells in the center of the projection. Re-run RNA velocity: did it change the trajectory of individual cells? Comment. 
...
p <- scater::plotReducedDim(sce2, 'PCA', colour_by = "AnnotatedClusters", point_alpha = 0.5) +
    geom_segment(
        data = grid.df2, 
        mapping = aes(x = start.PC1, y = start.PC2, xend = end.PC1, yend = end.PC2), 
        arrow = arrow(length = unit(0.05, "inches"), type = "closed")
